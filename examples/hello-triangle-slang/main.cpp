...
void init_app()
{
    ...
    state.window = glfwCreateWindow(
        init_width,
        init_height,
        "WebGPU Sandbox: Hello Triangle (Slang)",
        nullptr,
        nullptr);
    assert(state.window);

    // Create WebGPU context and report details
    state.gpu = GpuContext::make({state.window, "#hello-triangle-slang"});
    state.gpu.report();
    ...
    String buffer;
    const bool ok = read_text_file("assets/shaders/triangle.wgsl", buffer);
    assert(ok);
